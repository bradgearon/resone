...
receive({op:'composition',requestId:id,payload:{tracks:[{laneId:'a',notes:[{pitch:72,start:0,duration:1}],notation:'C5'}]}});
...
receive({op:'composition',requestId:id,payload:{tracks:[{laneId:'missing',notes:[{pitch:50,start:0,duration:1}]}]}});
assert(song.lanes[0].notes[0].pitch===72 && sent.filter(x=>x.op==='play').length===plays);
requestMusic('third');
receive({op:'error',requestId:pending.id,payload:{message:'bad notation'}});
assert(song.lanes[0].notes[0].pitch===72 && pending===null);
song.lanes.push({id:'b',name:'Bass',notes:[{pitch:40,start:0,duration:1}],notation:'E2'});
requestMusic('partial');
receive({op:'composition',requestId:pending.id,payload:{tracks:[{laneId:'a',notes:[{pitch:65,start:0,duration:1}],notation:'F4'}]}});
...
receive({op:'composition',requestId:pending.id,payload:{tracks:[{laneId:'a',notes:[{pitch:69,start:0,duration:1}],notation:'A4'}]}});
...
receive({op:'composition',requestId:pending.id,payload:{tracks:[{laneId:'b',notes:[{pitch:45,start:0,duration:1}],notation:'A2'}]}});
